...
export async function GET() {
  const [{ data: settings, error: settingsError }, { data: state, error: stateError }] =
    await Promise.all([
      supabaseAdmin
  ...
  return NextResponse.json({
    ...
    // 📣 Social auto-post (NEW / used by dashboard)
    enabled: settings.social_enabled, // used by AutoPublishPanel
