...
store_name: editDeal.store_name, 
      ...
      sub_category: editDeal.sub_category,
      holiday_tag: editDeal.holiday_tag,

      hash_tags: editDeal.hash_tags,
      hashtags_es: editDeal.hashtags_es,
      ...
      expire_date: editDeal.expire_date,

      is_affiliate: editDeal.is_affiliate,
      affiliate_source: editDeal.is_affiliate
        ? editDeal.affiliate_source
        : null,
      affiliate_priority: editDeal.is_affiliate
        ? editDeal.affiliate_priority
        : 0,
      affiliate_short_url: editDeal.is_affiliate
        ? editDeal.affiliate_short_url
        : null,
...
<input
  className="border p-2 rounded"
  placeholder="Hashtags (Spanish only, comma separated)"
  value={(editDeal.hashtags_es || []).join(", ")}
  onChange={e =>
    setEditDeal({
      ...editDeal,
      hashtags_es: e.target.value
        .split(",")
        .map(t => t.trim())
        .filter(Boolean),
    })
...
<p className="text-xs text-slate-500">
  Used only for Spanish posts. Generated by AI but editable.
</p>
...
<input
  className="border p-2 rounded"
  placeholder="Affiliate Short URL (e.g. https://amzn.to/...)"
  value={editDeal.affiliate_short_url || ""}
  onChange={e =>
    setEditDeal({
      ...editDeal,
      affiliate_short_url: e.target.value,
    })
  }
/>
